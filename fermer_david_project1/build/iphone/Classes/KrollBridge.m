...
NSString * Fermer_David_Demo$ModuleRequireFormat = @"(function(exports){"
...
@implementation Fermer_David_DemoObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_fermer_david_demo);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Fermer_David_Demo global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Fermer_David_Demo" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_fermer_david_demo = [[Fermer_David_DemoObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_fermer_david_demo];
	
	NSString *_fermer_david_demoNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _fermer_david_demoNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_fermer_david_demo valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Fermer_David_Demo$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.fermer_david_demo.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.fermer_david_demo.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
