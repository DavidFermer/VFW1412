...
NSString * Fermer_David_Final_Project$ModuleRequireFormat = @"(function(exports){"
...
@implementation Fermer_David_Final_ProjectObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_fermer_david_final_project);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_fermer_david_final_project gc];
}
...
{
	// create Fermer_David_Final_Project global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Fermer_David_Final_Project" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_fermer_david_final_project = [[Fermer_David_Final_ProjectObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_fermer_david_final_project];
	
	NSString *_fermer_david_final_projectNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _fermer_david_final_projectNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_fermer_david_final_project valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Fermer_David_Final_Project$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.fermer_david_final_project.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.fermer_david_final_project.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
