...
//Rare exceptions to indicate a bug in the _fermer_david_final_project code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_fermer_david_final_project = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_fermer_david_final_project]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.fermer_david_final_project.module"
