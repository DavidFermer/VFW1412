...
//Rare exceptions to indicate a bug in the _fermer_david_project3 code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_fermer_david_project3 = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_fermer_david_project3]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.fermer_david_project3.module"
