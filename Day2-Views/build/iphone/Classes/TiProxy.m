...
//Rare exceptions to indicate a bug in the _day2_views code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_day2_views = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_day2_views]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.day2_views.module"
